...
Esta animação cria um campo de trilhas em espiral com estética neon, profundidade volumétrica e sensação de fluxo contínuo.

O núcleo visual combina:

- Geometria procedural (`TubeGeometry` em curvas espirais)
- Shader customizado (movimento de trilha + onda no eixo Z)
- Pós-processamento (Bloom + FXAA)
- Controle em tempo real via GUI

Não é apenas um efeito decorativo: funciona como camada de ambientação para hero visual com forte percepção de tecnologia e movimento.

## MOTION READING

### Intenção visual

- Direção: fluxo para o centro (inward movement)
- Profundidade: combinação de fog + wave motion + bloom
- Ritmo: velocidade global ajustável por `uGlobalSpeed`
- Contraste: paleta fria/quente alternada entre 4 cores

### Comportamento da animação

1. Cada trilha recebe offset e velocidade próprios (`aOffset`, `aSpeed`)
2. O fragment shader calcula uma “janela” de cauda luminosa (`trailLength`)
3. O vertex shader injeta oscilação senoidal por UV para dar organicidade
4. Bloom amplifica brilho e FXAA reduz serrilhado no resultado final

### Decisões de performance

- Geometrias são mescladas com `BufferGeometryUtils.mergeGeometries`
- Atualização por frame atua principalmente em uniforms (`uTime`, `uGlobalSpeed`)
- Render final via `EffectComposer` com passes objetivos (RenderPass + Bloom + FXAA)

## USO RECOMENDADO

- Hero section de landing page com fundo escuro
- Contexto de produto tech/IA/dados
- Visual de transição entre seções conceituais

Evitar uso simultâneo com muitos elementos animados em foreground para não competir por atenção.

## CÓDIGO EXTRAÍDO (CODEPEN)

Fonte: https://codepen.io/asdkjbdbj/pen/ogzjYbB
